...
class MixTransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("IN VITsubmodules x.shape is ", x.shape)
        # print("len(self._attn) is ", len(self._attn))
...
class ConvMoudle(nn.Module):
    def __init__(self, in_channel, out_channel, kernel=1, stride=1, padding=0):
        super().__init__()
        self.c = nn.Conv2d(in_channel, out_channel, kernel, stride, padding)
        self.batch_norm = nn.BatchNorm2d(out_channel)
        self.act = nn.SiLU()
    
    def forward(self, x):
        x = self.batch_norm(self.c(x))
        return self.act(x)
        
class BottleNeck(nn.Module):
    def __init__(self, in_c, out_c, down_scale = False):
        super().__init__()
        self.c1 = ConvMoudle(in_c, in_c)
        if not down_scale:
            self.c2 = ConvMoudle(in_c, out_c, kernel=3, stride=1, padding=1)
        else:
            self.c2 = ConvMoudle(in_c, out_c, kernel=3, stride=2, padding=1)
    def forward(self, x):
