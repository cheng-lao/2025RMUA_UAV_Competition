...
class StereoDataset(data.Dataset):
                        #None   
    def __init__(self, aug_params=None, sparse=False, reader=None, temporal=False, 
                 frame_sample_length=4, is_test=False, ddp=False, load_flow=False, index_by_scene=False):
        self.augmentor = None          # none
        self.index_by_scene = index_by_scene    # True
        self.sparse = sparse        # True
        self.temporal = temporal    # True
        self.img_pad = aug_params.pop("img_pad", None) if aug_params is not None else None
        self.device = aug_params.pop("device", None) if aug_params is not None else None
        self.ddp = ddp            # False
        self.load_flow = load_flow  # False
    ...
    def __getitem__(self, index):
        ...
        if self.temporal:  # temporal loader: load data by image pair sequences
            ...
            if self.index_by_scene:  # first, index by scene (or sequences), then index by frame slice
                # sequence path
                index = index % len(self.image_list)    # len(self.image_list) == len(scenes)
                image1_list = self.image_list[index][0] # left  shape(frames, height, width, 3)
                image2_list = self.image_list[index][1] # right shape(frames, height, width, 3) 
                pose_list = self.pose_list[index]       # 每一帧相机的旋转矩阵和偏移向量 shape(frames, 4, 4)  
                disp_list = self.disparity_list[index]  # str: pathToScene
...
class KITTI(StereoDataset):
    def __init__(self, aug_params=None, root='datasets/KITTI', is_test=False, mode='single_frame', frame_sample_length=4,
                  image_set='training',ddp=False, index_by_scene=False, num_frames=11):
        
        super(KITTI, self).__init__(aug_params, sparse=True, reader=frame_utils.readDispKITTI, temporal=(mode == 'temporal'),
            frame_sample_length=frame_sample_length,is_test=is_test, ddp=ddp, load_flow=False, index_by_scene=index_by_scene)
        assert os.path.exists(root)
        if is_test:
            if mode == 'single_frame':
                ...
                print(os.path.join(root, image_set, 'sequences', '**'))
                ...
                for scene in scene_list:
                    # scene 是一个文件夹路径，eg. /home/chengzi/ProgramData/Temporally-Consistent-Stereo-Matching/datasets/KITTI/kitti_seq/kitti2015_testings/sequences/000198
                    # image1_list 是二维列表，每个元素存储一组图片
