...
{  
    ...
    velcmd.twist.linear.y = 0.01;//y方向线速度(m/s)
    velcmd.twist.linear.z = 0.02; //z方向线速度(m/s)

    //无人机信息通过如下命令订阅，当收到消息时自动回调对应的函数
    odom_suber = nh->subscribe<geometry_msgs::PoseStamped>("/airsim_node/drone_1/debug/pose_gt", 1,
         std::bind(&BasicDev::pose_cb, this, std::placeholders::_1));//状态真值，用于赛道一
    
    gps_suber = nh->subscribe<geometry_msgs::PoseStamped>("/airsim_node/drone_1/gps", 1,
         std::bind(&BasicDev::gps_cb, this, std::placeholders::_1));//状态真值，用于赛道一
    ...
    takeoff_client = nh->serviceClient<airsim_ros::Takeoff>("/airsim_node/drone_1/takeoff");//起飞
    land_client = nh->serviceClient<airsim_ros::Takeoff>("/airsim_node/drone_1/land");//降落
    reset_client = nh->serviceClient<airsim_ros::Reset>("/airsim_node/reset");//重置
    //通过publisher实现对无人机的速度控制和姿态控制和角速度控制
    vel_publisher = nh->advertise<airsim_ros::VelCmd>("airsim_node/drone_1/vel_cmd_body_frame", 1);
    
    this->takeoff.request.waitOnLastTask = 1;
...
//获取 orientation的四元数，然后转换成欧拉角，再获取位置信息
...
{
    // printf("Get front left view image\n height: %d\n weight: %d\n size: %ld\n",
...
//获取前右视图，这里的msg是一个图像数据
void BasicDev::front_right_view_cb(const sensor_msgs::ImageConstPtr& msg)
{
    // printf("Get front right view image\n height: %d\n weight: %d\n size: %ld\n",
    // msg->height, msg->width, msg->data.size());//图像高度宽度以及数据大小， size = height*weight*3 (8bit3通道)
    cv_front_right_ptr = cv_bridge::toCvCopy(msg, sensor_msgs::image_encodings::TYPE_8UC3); //将图像数据转换成opencv格式
